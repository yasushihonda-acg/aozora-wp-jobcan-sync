...
| `.claude/memory/illustration-baseline.png` | フルシーン baseline (nurse, タブレット相談) |
| `.claude/memory/illustration-baseline-character-closeup.png` | 顔クローズアップ baseline |
...
**baseline 画像 3 点の差し替えは decision-maker (本田様) 領分。番号単位明示認可なしに改変禁止。**
...
- 動作 (e.g. "serving turquoise mug on a wooden tray")
...
```bash
bash .claude/skills/aozora-illust/scripts/gen.sh --mode=sheet
```
...
- ✅ 髪型 + 眼鏡 (べっ甲) + ピアス (ゴールドフープ)
- ✅ 画風 (繊細線画 + 透明水彩淡彩、シェーディング最小)
- ✅ 色調 (ターコイズ #00c4cc 中心、暖色肌、淡い緑背景)
- ❌ テキスト混入なし
...
- ❌ baseline 画像 3 点を断りなく差し替え
- ❌ prompts/*.txt を「もっと良くなりそう」で自己判断で改変
- ❌ シーン記述に「young」「cute」「anime」「3D」「glossy」を含める
