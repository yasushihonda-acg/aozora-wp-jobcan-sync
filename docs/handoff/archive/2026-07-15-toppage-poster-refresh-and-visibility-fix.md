...
## アーカイブ: 完了ミッション「トップページのトンマナ全面ブラッシュアップ」(GOAL.md 2026-07-15、達成につき置換)

上記セッション終了後、決裁者からの実機フィードバックを受けて `docs/handoff/GOAL.md` に以下のミッションが設定され、全項目達成した (次の新ミッションに置き換えるため、消失防止のためここに全文転記)。
...
### 完了の定義 (全達成)
- [x] `mockup/assets/img/sky-hero.jpg` が Image #2 由来の新イラストに置き換わっている
- [x] `.hero__title em` のマーカー背景が除去され、ヒーロー文字が判読できる
- [x] `illust-numbers.jpg` への参照が index.html から消えている
- [x] 「入社からのキャリアアップモデル」セクションが index.html に存在する
- [x] スクロールリビール実装 `mockup/assets/js/site.js` が存在し index.html から読み込まれている
- [x] ブラウザ console エラー 0 件
- [x] `docs/specs/chatgpt-ui-prompts.md` と `CLAUDE.md` の記述が実装内容と整合

### 実施内容サマリ
- PR #61: ヒーロー視認性修正(マーカー除去+ディレクショナルスクリム)・「入社からのキャリアアップモデル」新設・Numbersセクションの帯レイアウト化・スクロールフェードイン実装。`/code-review medium` + `codex review` 計6件の指摘(WCAGコントラスト・JS非表示リスク等)を全修正
- PR #62: ヒーロー背景画像をImage #2の新バリエーションに差し替え
- PR #63: ヒーロー/entry-cta背景の `cover` クロップで都市スカイラインが切れる問題を `background-position: center bottom` で修正
- 全PRとも GitHub Pages 本番で実機確認済み

### 次ミッションへの引き継ぎ
このミッション完了後、決裁者から「コーポレートカラーをあえて外し、イラスト世界観の配色に統一 + 視差効果強化 + AI臭さの払拭」という第2フェーズの方針転換指示があり、新ミッションとして `docs/handoff/GOAL.md` に書き換え済み(本アーカイブと同日)。
