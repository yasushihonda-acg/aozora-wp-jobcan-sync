...
## 🔄 中断点（in-flight）
- **CSVデータ取得方式への移行検討(2026-08-10開始)**: 決裁者から「求人カテゴリの粒度不一致」指摘を起点に、HTML解析方式の限界(反映ラグ・新カテゴリ取りこぼし・解析誤り・募集終了判定の間接推測)への懸念が拡大し、ジョブカン管理画面のCSV出力を自動取得する方式への切り替えを検討。設計はほぼ固まり、Playwright PoCで実現可能性を実証済み(本田氏がログイン・実行ボタンを担当、AIはナビゲーションのみ担当する安全な役割分担で実施。求人カテゴリ・給与等CSV列の構造検証、施設コード→名称の対応表確立(拠点一覧`/configs/branches`から27拠点分取得、スクラッチパッドに`facility_codes.py`として保存済み・未コミット)まで完了)。**次の一手**: システム部へ依頼した専用アカウント(`jobcan-sync@aozora-cg.com`)がGoogleグループとして発行されており、ジョブカンからの招待メールが届いていない不具合が発生、システム部へ対応依頼中でブロック。解決したら①jobcan-sync@aozora-cg.comの受信箱で招待メール確認→パスワード設定→ログイン確認 ②同アカウントでの求人一覧CSV取得手順の再現確認(閲覧限定権限でも動作するか) ③Secret Managerへ認証情報登録 ④Playwright自動化のコード実装(CLI化)、の順で再開。自動化の利用規約についてジョブカンへ照会する案は「自社の正規アカウントでの自動操作は通常運用の範囲」との判断で不要と結論済み(照会不要)
