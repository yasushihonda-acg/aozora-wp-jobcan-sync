...
- [x] **② 条件+地図検索(GPS、市区町村レベル、13拠点)**(PR #76→#78→#80、2026-07-23完了・マージ済み): 求人一覧ページ(`jobs.html`)に職種/雇用形態/エリア/フリーワードのANDフィルタ + 九州シルエット地図(13拠点) + GPS現在地からのHaversine距離順並べ替えをプログレッシブエンハンスメントで追加。既存34求人カードのマークアップは無改変
  - **地図表現の刷新(PR #78→#80、2段階)**: PR #76当初はLeaflet.js+国土地理院タイル(標準地図→淡色地図→白地図)を採用したが、決裁者フィードバック「実地図画像である限りトンマナと合わない」を受けPR #78で完全廃止、福岡/鹿児島の相対位置だけを保持した自作の抽象ブロブ図に置き換え。さらに決裁者が参考画像(九州県境入りフラット単色シルエット)を共有し「スタイリッシュな感じが良い、色はサイトデザインに合わせて」と指示、PR #80で `assets/img/kyushu-map.svg`(geolonia/japanese-prefectures 由来、Wikipedia日本地図.svgベースのGFDL、九州7県のみ抽出・鹿児島は本土+近隣島嶼のみ)による実データベースの正確な地図シルエットへ再刷新。拠点のある福岡・鹿児島の2県のみサイトのコバルトブルーで強調、ピンは各県重心(SVG viewBox比%)にクラスタ配置。外部CDN依存は完全になくなり、GPS距離計算(Haversine)は実緯度経度のまま可視化のみ分離。`/code-review high`を計3ラウンド実施(指摘は都度修正、直近はレターボックスによるピン位置ズレ・非同期ピン生成の競合状態を修正)
