...
- [x] **② 条件+地図検索(GPS、市区町村レベル、13拠点)**(PR #76、2026-07-23完了・マージ済み): 求人一覧ページ(`jobs.html`)に職種/雇用形態/エリア/フリーワードのANDフィルタ + Leaflet.js+国土地理院タイルの拠点地図(13拠点マーカー) + GPS現在地からのHaversine距離順並べ替えをプログレッシブエンハンスメントで追加。既存34求人カードのマークアップは無改変。Evaluator全AC(6項目)PASS・`/code-review high`実施(5件指摘、全修正済み: falsy-zero距離バグ・未ガード参照・ハイライト残留・デッドコード・地図初期化失敗時のレイアウト崩れ)
  - 拠点座標: 正本データ(`scripts/mockup-rebuild/jobs_data.json`の番地レベル住所)を国土地理院AddressSearch APIで一度きりジオコーディングし`scripts/mockup-rebuild/build_geo_data.py`にハードコード。求人34件はカテゴリ内訳 介護20/事務10/IT4(看護0、①と同じ既知制約)
  - JS無効 / Leaflet CDN障害時は既存34カード全表示にフォールバック(Playwright実機検証済み)
