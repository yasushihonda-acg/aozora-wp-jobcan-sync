...
## 完了の定義 (トップページ文言・改行修正 決裁者フィードバック5件) — 2026-08-21 実装完了・本番デプロイ完了・実機確認完了・PR #222
セッション冒頭でジョブカンCSV自動取得の6時間ごと同期(`aozora-sync-daily-trigger`)の健全性を実測確認(直近実行とも`Completed/True`、`ats_errors=0`/`crawl_errors=0`)、問題なし。続けて決裁者からトップページのスクリーンショット添付でヒーロー見出し・コピー文言の修正依頼を段階的に受け、都度実装・実機確認した。
- [x] ヒーロー見出し1行目「ケアスタッフに、」の末尾読点削除
- [x] ヒーロー見出し2行目の改行位置修正を2段階で試行: まず`word-break: keep-all`+`text-wrap: balance`で「を。」の孤立改行を解消したが、決裁者から「2行に収まっていない」と再指摘を受け、句読点自体を外す方針(「やりがいと経済的豊かさを」)へ変更して2行構成を達成。CSS追加分は保険として残置
- [x] ヒーロー直下リード文「介護・看護・在宅支援」→「包括的なケアネットワーク」
- [x] Philosophyセクション「経済的リターンを得られるようにしたいと考えています。」→「得られないとダメだと考えています。」
- [x] ミッションカード04「育てあう文化を、当たり前に」→「「育つ」のハードルを低くする」(本文も資格取得費用の全額補助・MBA研修等へ差替え)
- 検証方法: `sync/src/sync/app.py`の`_render_top_page()`を直接実行しCloud Run配信時の実レンダリング結果を確認、Playwrightでスクリーンショット比較。ローカル検証用の一時ファイル(`mockup/_verify_scratch.html`等)はリポジトリに残さず都度削除
- pytest/lint等の自動テストは対象外(純粋なコンテンツ変更のためCLAUDE.mdのcodex review必須基準=実コード3ファイル以上/100行以上に未達)
- **判明した既知の制約(記録のみ、対応不要と決裁者確認済み)**: `/assets/*`静的ファイルは`Cache-Control: max-age=3600`かつファイル名にcontent-hashなし。デプロイ直後は「そのセッション中に一度でもサイトを開いたブラウザ」で最大1時間古いCSSが表示され続ける(HTMLの文言自体は都度取得のため即時反映、影響はCSSのみ)。AskUserQuestionで対応要否を確認し「様子見」を選択、Issue化もせず

## 完了の定義 (チャットボット知識ベース本番機能低下の修正 + Stage 5事前監査) — 2026-08-14 実装完了・本番デプロイ完了・実機確認完了・PR #217〜#220
GitHub Issues/条件待ちタスクに即着手なしの状態で、decision-makerから「ROI良好な内容がないか」との確認を受け、decision-maker判断不要な低リスク改善候補(依存関係・codex見送り項目・テストカバレッジ・本番疎通)を調査した結果、**実害のある本番バグを発見**。続けて「最終仕上げ」の位置づけでFable 5によるドメイン切替(Stage 5)前提の全体監査を実施し、切替時の設定更新漏れを1件発見・修正した。
- [x] **チャットボット知識ベースのコールドスタートタイムアウト修正(PR #217〜#218)**: 本番`aozora-chatbot`の`/health`を実測し`{"source":"bundled","job_count":0}`(求人0件・FAQのみ状態)を発見。Cloud Runログを`chatbot`/`sync`双方で突き合わせ、両サービスとも`minScale`未設定でscale-to-zeroするため低トラフィック帯で同時にコールドスタートすると`sync`側の起動完了(実測約3.3秒)が`chatbot`側のfetch timeout(3.0秒)より遅く知識ベース取得が失敗することを秒単位のログ相関で特定(2026-08-14 04:49:57.769〜04:50:00.784 JST)。加えて失敗後は次の再試行まで最大1時間(`knowledge_refresh_interval_seconds`)求人0件のままという設計上の二次問題も判明。`knowledge_fetch_timeout_seconds`を3.0→10.0秒に緩和し、新設の`knowledge_refresh_retry_interval_seconds`(既定300秒)で失敗後の再試行間隔を短縮。CLAUDE.mdのチャットボット節が2026-07-26時点の古い設計記述(GitHub Pages `jobs_detail.json`フェッチ方式)のままだったことも合わせて発見・修正(PR #217、実際は2026-08-09 PR #154でFirestore直結済みだった)。pytest 92件全PASS(新規回帰テスト7件)、ruff/pyright 0件、codex review(`--strict-config`含む2回実行)findings 0件。本番デプロイ後`/health`で`{"source":"fetched","job_count":382}`を実機確認済み
- [x] **Stage 5(ドメイン切替)事前監査 + 実行チェックリスト新設(PR #219〜#220)**: decision-maker確認を経てFable 5へ一時切替し、ドメイン切替前提の全体監査を実施。Maps APIキー`jobs-map-embed`のリファラー許可リストに最終ドメイン`recruit.aozora-cg.com`が未登録で、かつ既存のStage 5リマインダー(`PUBLIC_BASE_URL`・`ALLOWED_ORIGINS`)のどちらにも紐付いていない漏れを発見(実測確認、放置すると切替日に新ドメインの`/jobs/`で地図だけがサイレントに壊れる)。対応として`infra/README.md` §10に前提条件→当日手順9件→判断項目(`min-instances=0`の公開時見直し等)→ロールバックの統合チェックリストを新設し、既存リマインダー3箇所へ相互参照を追記。監査完了後`/model opusplan`でSonnet 5ベースへ復帰済み

## 完了の定義 (求人一覧: 職種別ページへの検索パネル追加 + 表示ラグ解消) — 2026-08-14 実装完了・本番デプロイ完了・実機確認完了・PR #210〜#215
...
- [x] **フォローアップ1(PR #212)**: 本番実機を見た決裁者から「全求人ページで職種がダブっている」と報告。`.job-search-nav`を全求人ページ(`not page.category_id`)にも出していたため、既存の職種チップ行(パネル内、複数選択)と同じ17職種が2箇所に重複表示されていた。ナビを`page.category_id`が真の場合(職種別ページ)のみに限定して解消。ドッグフーディング(実際に数職種クリックして回遊)では見落とし——職種別ページの遷移体験しか見ておらず、変更が影響する「もう一方のモード」(全求人ページ)自体をスクリーンショットで見ていなかったのが原因(教訓は`~/.claude/memory/feedback_ui_doc_must_visit_real_ui.md`の「派生ケース」として汎用メモリへ統合済み)
- [x] **フォローアップ2(PR #213)**: 決裁者提案「職種別ページのナビを全求人ページの『職種』パネル行と統一したデザインにできないか」を受け、ナビを`.job-search-panel__row`/`__label`/`__chip`へ全面差し替え、`#job-search-panel`に隣接させてCSSで1枚のシームレスなカードに統一。実装過程でcodex review(P2)が「`.job-search-nav + .job-search-panel`の隣接セレクタが、navとpanelの間にインラインJSON/エラーメッセージ/noscriptが挟まっていて実際にはマッチしない」という実バグを検出、該当要素をnavの前に並べ替えて隣接を保証(computed styleで`border-radius`/`box-shadow`の切り替わりを確認)。本番反映確認時、静的アセットの1時間キャッシュ(`Cache-Control: public, max-age=3600`)によりPlaywrightセッション・decision-maker双方で一時的に旧デザインが見えたが、`fetch(..., {cache:'no-store'})`でサーバー配信自体は正しいことを確認、時間経過で解消
- [x] **別件バグ発見・修正(PR #214, #215)**: 決裁者が求人詳細ページ「選考の流れ」のスクリーンショットで「番号バッジの位置がズレている」と報告(今回のセッション作業とは無関係の既存バグ)。調査の結果2つの独立した原因が判明: ①(PR #214) CSS詳細度の競合——本文`<ul>`用の汎用箇条書きマーカー`.job-detail-section__body li::before`(8px、`li`全称セレクタ)が、`<ol class="selection-flow">`のstep liにも意図せずマッチし、詳細度で専用スタイル`.selection-flow__step::before`(36px・番号入り円)を上書きしていた。セレクタを`ul li::before`に絞り込んで解消 ②(PR #215) CSS修正後に発覚した番号の二重表示——Jobcan投稿の生テキストに担当者が手入力した丸数字(①②③④、投稿ごとに有無混在)と、CSSが描画する連番バッジが両方表示されていた。`detail_sections.py::extract_selection_flow`と`rewrite_job_details.py`(Phase Aモック生成)の両方に先頭丸数字除去の正規化を追加、Phase Aモック(`mockup/jobs/*.html`)2件も同期。pytest 556件全PASS、codex review(medium+high effort)+ pr-review-toolkitセカンドオピニオンいずれもfindings 0件。本番デプロイ・実機確認済み(revision `aozora-sync-00031-tv6`)
...
- ~~相談員イラストカード化~~ → **2026-08-13 完了(PR #186)**。SCENE #21で3枚生成、うち1枚(高齢の利用者様・ご家族と3人でテーブルを囲みタブレット提示するシーン)は採点の過程で既採用済み`illust-job-consultant.png`/`-2.png`(ケアマネジャー)と構図がほぼ同一(3人・テーブル囲み・タブレット提示のポーズまで酷似)と判明し不採用(PR #183と同じ構図drift再発パターン)。残り2枚のうち施設内案内の歩行シーンを`illust-job-consultation.png`としてトップページ「相談員」カードに採用、窓口一対一相談シーンを`illust-job-consultation-2.png`としてバリエーション採用。`selectors.yaml`に`consultation`バケット新設(`consultant`から「相談員」synonymのみ分離・移動、ケアマネジャー・サービス管理責任者の既存割り当ては無変更)。pytest 544件全PASS、ruff/pyright 0件、codex review(effort high)+ pr-review-toolkit セカンドオピニオンいずれもfindings 0件。Cloud Run Job・Webサービスとも再デプロイ・実機確認済み(トップページカード表示・`category_id=18984`がconsultation系2枚のみに分散・`18985`(ケアマネジャー)/`22014`(サービス管理責任者)の既存割り当てがベースラインと完全一致、console error 0件)。テキストタグから格上げ(7→6)。残り6職種(サービス提供責任者4件・サービス管理責任者6件・世話人6件・訪問リハビリ6件・サポート職7件・新卒既卒2件、2026-08-12実測)のうち、件数最多のサポート職(清掃・洗濯・調理・送迎)は **2026-08-13 完了(PR #189)**。SCENE #23で3枚生成、うち2枚(清掃シーン・洗濯シーン)は耳元拡大確認で小さなスタッドピアスが視認され不採用(ACCESSORY RULEを無条件「NO earring」で指定していても発生しうると判明)、洗濯シーンはさらに既存`illust-job-visit.png`(訪問介護員、洗濯物たたみ)と構図が重複し二重の不採用理由。残り1枚(送迎車へのご案内シーン)を`illust-job-support.png`として単独採用(1枚プール、`facility-manager`と同じ先行反映パターン、清掃/調理のバリエーション追加は次セッション以降のフォローアップ)。`selectors.yaml`に`support`バケット新設(`care`から「サポート職」synonymのみ分離・移動、介護職・世話人の既存割り当ては無変更)。pytest 544件全PASS、ruff/pyright 0件、codex review(effort high)+ pr-review-toolkit セカンドオピニオンいずれもfindings 0件。Cloud Run Job・Webサービスとも再デプロイ・実機確認済み(トップページカード表示・`category_id=43764`がsupport系1枚のみに反映・`18773`(介護職)の既存割り当て[care.png/-2/-3]がベースラインと完全一致、console error 0件)。テキストタグから格上げ(6→5)。残り5職種(サービス提供責任者4件・サービス管理責任者6件・世話人6件・訪問リハビリ6件・新卒既卒2件)のうち、件数同率1位の訪問リハビリは **2026-08-13 完了(PR #192)**。SCENE #24で1枚生成(リハビリバーを使った上肢運動療法シーン)、耳元拡大確認+10項目採点とも全Pass、既存`illust-job-nurse.png`(医療行為)・`illust-job-visit-3.png`(歩行介助)と視覚的に完全に区別できるため`illust-job-visiting-rehab.png`として単独採用(1枚プール、`facility-manager`/`support`と同じ先行反映パターン)。`selectors.yaml`に`visiting-rehab`バケット新設(`nurse`から「訪問リハビリ」synonymのみ分離・移動、看護職・看護師の既存割り当ては無変更)。pytest 544件全PASS、ruff/pyright 0件、codex review(effort high)+ pr-review-toolkit セカンドオピニオンいずれもfindings 0件。Cloud Run Job・Webサービスとも再デプロイ・実機確認済み(トップページカード表示・`category_id=41046`がvisiting-rehab系1枚のみに反映・`18983`(看護職)の既存割り当て[nurse.png]がベースラインと完全一致、console error 0件)。テキストタグから格上げ(5→4)。残り4職種(サービス提供責任者4件・サービス管理責任者6件・世話人6件・新卒既卒2件)のうち、件数同率1位のサービス管理責任者(世話人と同率、decision-makerタイブレークで選定)は **2026-08-13 完了(PR #197)**。SCENE #25で2枚生成(デスクでタブレット+ペンを持つシーン/廊下を書類持って歩くシーン)、10項目採点+耳元拡大確認とも全Pass。既存カードとの構図比較で廊下シーンが`illust-job-general-2.png`(総合職バリエーション、同一の窓+観葉植物+受付背景の廊下歩行構図)と酷似と判明し不採用。デスクシーンは`illust-job-facility-manager.png`(座り姿勢+片手タブレット+片手ペンの共通アーキタイプ)と要素は近いが、背景(オフィスデスク+都市スカイライン窓 vs ロビー受付+階段)とトーンが明確に異なるためdecision-maker判断で視覚的区別可能と採用、`illust-job-service-manager.png`として単独採用(1枚プール、`facility-manager`/`support`/`visiting-rehab`と同じ先行反映パターン)。`selectors.yaml`に`service-manager`バケット新設(`consultant`から「サービス管理責任者」synonymのみ分離・移動、ケアマネジャー・計画作成担当者の既存割り当ては無変更)。pytest 544件全PASS、ruff/pyright 0件、codex review(effort medium)+ pr-review-toolkit セカンドオピニオンいずれもfindings 0件。Cloud Run Job・Webサービスとも再デプロイ・実機確認済み(トップページカード表示・`category_id=22014`がservice-manager系1枚のみに反映・`18985`(ケアマネジャー)の既存割り当て[consultant.png]がベースラインと完全一致、静的画像疎通200。**本セッションはPlaywright MCPが利用不可のためcurlベースのHTTP/HTML検証に限定、console error確認は未実施**)。テキストタグから格上げ(4→3)。残り3職種(サービス提供責任者4件・世話人6件・新卒既卒2件)のうち、件数最多(6件)の世話人は **2026-08-13 完了(PR #200)**。SCENE #26で2枚生成(グループホームでの食事配膳シーン/脱衣所でのタオル手渡しシーン)、10項目採点+耳元拡大確認を実施。脱衣所シーンは背景に浴槽・シャワー・介護用いすが明確に描かれSCENE本文で明示的に禁止した入浴・身体接触の構図に直接抵触し不採用(既存カード比較以前にSCENE指示違反)。食事配膳シーンはCOMPOSITIONで例示した3パターン(服薬確認/買い物リスト/お茶)のいずれでもなく「二人とも着席」の指定からも外れる(スタッフは立位)が、companionshipの精神には合致し既存カードとの構図重複もないためdecision-maker判断で採用、`illust-job-caretaker.png`として単独採用(1枚プール、`facility-manager`/`support`/`visiting-rehab`/`service-manager`と同じ先行反映パターン)。`selectors.yaml`に`caretaker`バケット新設(`care`から「世話人」synonymのみ分離・移動、介護職の既存割り当ては無変更)。pytest 544件全PASS、ruff/pyright 0件、codex review(effort medium)+ pr-review-toolkit セカンドオピニオンいずれもfindings 0件。Cloud Run Job・Webサービスとも再デプロイ・実機確認済み(トップページカード表示・`category_id=39695`がcaretaker系1枚のみに反映・`18773`(介護職)の既存割り当て[care.png]がベースラインと完全一致、静的画像疎通200。**本セッションはPlaywright MCPが利用不可のためcurlベースのHTTP/HTML検証に限定、console error確認は未実施**)。テキストタグから格上げ(3→2)。残り2職種(サービス提供責任者4件・新卒既卒2件)のうち、件数最多(4件)のサービス提供責任者は **2026-08-13 完了(PR #204予定)**。SCENE #27で1枚生成(高齢の利用者様・ご家族・スタッフの3人がテーブルを囲みタブレットを提示するシーン)、当初COMPOSITIONが必須指定した壁掛け地図/スケジュールボードが描かれず10項目チェックの項目9(職務内容整合)がFail、既存`illust-job-consultant.png`系/`illust-job-consultation.png`系との構図近似も判明したため一旦再生成用に絞り込みを実施(PR #203)。しかし本田様が「現代のオフィスでは壁に地図やスケジュールボードを掲示する運用は実態にそぐわない」と判断し、絞り込み前の初回生成画像をそのまま採用する decision-maker override を実施(項目9は decision-maker 判断で Pass 扱いへ変更、既存カードとの構図近似は許容)。`illust-job-service-lead.png`として単独採用(1枚プール、`facility-manager`/`support`/`visiting-rehab`/`service-manager`/`caretaker`と同じ先行反映パターン)。`selectors.yaml`に`service-lead`バケット新設(`visit`から「サービス提供責任者」synonymのみ分離・移動、ホームヘルパーの既存割り当ては無変更)。テキストタグから格上げ(2→1)。**教訓: SCENE COMPOSITIONが前提とする業務環境の実在性(物理的な小道具の現代における実態)はAI側の想定が実態と乖離しうる、decision-makerの現場知識による最終判断を機械的Fail判定より優先する**。pytest 544件全PASS、ruff/pyright 0件、codex review(effort medium)findings 0件。Cloud Run Job・Webサービスとも再デプロイ・実機確認済み(トップページカード表示・`category_id=18990`がservice-lead系1枚のみに反映・`18986`(ホームヘルパー)の既存割り当て[visit.png/-2/-3]がベースラインと完全一致、Playwright MCPで console error 0件確認、スクリーンショットでカード表示・「その他の募集職種」タグから除去を目視確認)。残り1職種(新卒既卒2件)は **2026-08-13 完了(PR #206)**、これで決裁者指摘の11職種すべてが専用イラストカード化完了。SCENE作成を経ず本田様が直接ChatGPT UIで1枚生成(黒/charcoalテーラードジャケット+白襟シャツのスーツ姿でガラス張りオフィスロビーを書類フォルダを片手に歩くシーン)、10項目採点のうち項目9(職務内容整合)は既存`illust-job-general-2.png`(総合職バリエーション、ガラス張り廊下を歩く構図)とのアーキタイプ近似が判明したが、衣装(スーツ+白襟シャツ vs 黒ポロシャツ)による職務の質的差別化を理由にdecision-maker判断で採用。`illust-job-new-grad.png`として単独採用(1枚プール、`facility-manager`/`support`/`visiting-rehab`/`service-manager`/`caretaker`/`service-lead`と同じ先行反映パターン)。`selectors.yaml`に`new-grad`バケット新設(`office`から「新卒・既卒総合職」synonymのみ分離・移動、事務職の既存割り当ては無変更)。トップページの「その他の募集職種」テキストタグセクションは対象消滅により削除。**教訓: 構図の共通アーキタイプが既存カードと重複していても、衣装や小道具といった別の視覚要素で職務の質的差異を明確に表現できていればdecision-maker判断で許容されうる**。pytest 544件全PASS、ruff/pyright 0件、codex review(effort medium)findings 0件。Cloud Run Job・Webサービスとも再デプロイ・実機確認済み(トップページカード表示・`category_id=73697`がnew-grad系1枚のみに反映・`58859`(事務職)の既存割り当て[office.png/-2]がベースラインと完全一致、Playwright MCPでconsole error 0件確認、スクリーンショットでカード表示・「その他の募集職種」タグ消滅を目視確認)
- ~~募集中の職種カードのスクロール出現アニメーションのタイミング修正(PR #208)の本番反映~~ → **2026-08-13 完了**。マージ時点でTest planの「Cloud Runの再デプロイ・本番実機確認」が未消化のまま残っていた中断点。`docker buildx build`→`gcloud run deploy`で本番反映(`aozora-sync-00026-f7b`)、Playwright実機確認で3列(1440px)/2列(768px)/1列(375px)いずれの幅でも各ページ読み込み時点のtransition-delayが列位置基準で`0/130/260ms`(3列)・`0/130ms`(2列)・`0ms`固定(1列)と正しく循環することを確認、console error 0件。**判明した注意点**: delay計算はページ読み込み時に1回だけ実行される設計(リサイズイベントでの再計算は非対応)のため、ブレークポイント別の検証は同一セッション内でのウィンドウリサイズではなく、各幅での`navigate`(フルリロード)で行う必要がある
