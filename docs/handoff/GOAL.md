...
- [ ] ② 条件+地図検索(GPS、市区町村レベル、12拠点) — 新規アーキテクチャ判断、フルplan要。**規模感調査完了(2026-07-23)、decision-maker共有・判断待ち**:
  - 拠点マスタ: 実データ(jobs.html等)から施設名+市区町村を確認済み(鹿児島市7+福岡地区6=計13拠点、コンサル提示「12拠点」とほぼ一致)。緯度経度・番地レベル住所は未整備、一度きりのジオコーディング作業が必要
  - 地図ライブラリ: **Leaflet.js + 国土地理院タイル**を推奨(一次ソース確認済み、商用利用可・APIキー不要・無料、出典表示必須)。OSM生タイルは公式ポリシーで商用継続利用が非推奨と明記のため不採用。Google Maps APIは月10,000回表示まで無料だが2025-03に定額$200クレジット制度廃止・従量課金化済み
  - GPS最寄り拠点: ブラウザ標準Geolocation API + Haversine距離計算のみで実現可(追加コストなし)
  - 条件検索+フリーワードUI: `components.css` に `.job-list-filter` のCSSクラスは定義済みだが**現状mockup内で未使用(実装UIなし)**、ゼロから実装が必要
  - 技術コストは実質ゼロ(全て無料枠/OSS)。5〜8ファイル規模の新機能、フルplan必須
