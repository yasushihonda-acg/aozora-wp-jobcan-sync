...
## 8. 一覧ページ (`/aozora/list?category_id=...`) 構造 (Phase 2A.1a 追加検証)
...
|---|---|
| 求人カード | `.job-offer-box` |
| 個別求人 URL | `a[href^="/aozora/job_offers/"]` |
| 求人タイトル | `.job-offer-title` (h2) |
| 拠点 | `.job-offer-address` |
| 雇用形態 | `.job-offer-label` (ul) |
| 概要 | `.job-offer-description` |
| ページネーション | `.content-box-wide.pagination-area` |

### 検証済カテゴリ

| category_id | 名称 (一覧 title) | 求人数 (Phase 2A.1a 取得時) |
|---|---|---|
| 18773 | 介護職 求人一覧 | 10 件 |
| 18984 | 相談員 求人一覧 (※「看護師」想定だったが実際は「相談員」) | 10 件 |
| 69384 | IT エンジニア職 求人一覧 | 4 件 |
| 58859 | 事務職 求人一覧 | 10 件 |

> ⚠️ category_id 18984 は当初「看護師」と推測したが、実際は「相談員」だった。category_id → 職種名のマッピングは Phase 2A.1b の一覧ページ実装時に再検証する。

### 検証済 fixtures (5 件、Phase 2A.1a で取得)

| job_id | 職種・雇用形態 | category_id |
|---|---|---|
| 1777023 | 介護職 正社員 (博多／デイ・有料) | 18773 |
| 1668696 | 相談員 短時間正社員 (永吉) | 18984 |
| 1690435 | IT エンジニア職 正社員 (社内 SE アシスタント) | 69384 |
| 2215694 | 事務職 正社員 (鹿児島市小野) | 58859 |
| 2199420 | 介護職 正社員 (田村) | 18773 |

> 注: 「パート」「アルバイト」雇用形態の fixture は Phase 2A.1a では取得できず (アクセスした 4 カテゴリ内に正社員系のみ確認)、Phase 2A.1b の一覧ページ実装時に再探索する。

## 9. 今後追加予定の検証

Phase 2A.1b で実施:
- カテゴリ一覧ページのページネーション (`?page=2` 等のパラメータ確認)
- パート / アルバイト求人の category_id 特定 + fixture 取得
- トップページ (`/aozora`) のメインビジュアル / カテゴリ一覧 / 地域別一覧

Phase 2A.2 で実施:
- FastAPI app に list / detail エンドポイント追加
