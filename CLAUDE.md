...
- 知識ベース (FAQ5件+求人サマリー+求人詳細) はRAGなし。FAQは同梱データ固定 (`index.html #faq` 更新時は `faq.yaml` の手動同期が必要)。求人データは同梱ファイルを持たず (2026-08-09 PR #154 で削除)、`sync` (Cloud Run) の `GET /jobs/chatbot-knowledge.json` を唯一の情報源として取得する (下記参照)
...
- **2026-08-09 知識ベースをPhase B (Firestore実データ) へ直結 (PR #154)**: 2026-07-26時点のGitHub Pages `jobs_detail.json` フェッチ方式 (下記は履歴) がPhase A静的37件データのまま更新停止し、サイト本体 (Firestore、6時間ごと自動同期) と情報不整合を起こしていた本番不具合を解消。`sync` 側に `GET /jobs/chatbot-knowledge.json` (`build_chatbot_knowledge`) を新設し、Firestoreスナップショットからchatbot向け形状をリクエスト時に都度生成 (ビルド成果物ではなく常に最新)。chatbot側は同梱の `jobs_detail.json` + 手動更新スクリプト (`build_jobs_detail.py`) を完全削除し、`DEFAULT_JOBS_DETAIL_URL` を上記エンドポイントへ切替。リフレッシュは `/chat` リクエスト先頭で経過時間を確認し必要なら同期的に再取得する**リクエスト駆動方式** (`knowledge._maybe_refresh_knowledge` 相当、`app.py`) — 当初実装したasyncioバックグラウンドタイマー方式はCloud Runの既定CPU割り当て (リクエスト処理中のみ) の下で実機動作しないとcodex reviewで判明し再設計した経緯あり。取得データは`url`を信頼せず`id`から再計算 (`chat-widget.js`が`job.url`をそのまま`<a href>`に使うため)、改行/`|`/制御文字を含むレコードは拒否。fetch失敗時は古いサンプルデータに黙ってフォールバックせず、FAQのみの正直な応答に切り替える (`bundled_knowledge()`)。これにより手動反映ステップ (`build_jobs_detail.py` 実行 + `git push`) は不要になり、6時間ごとの自動同期に完全追随する (詳細: `chatbot/README.md`)
  - (履歴) 2026-07-26時点は `chatbot/src/chatbot/knowledge/jobs_detail.json` をコンテナ同梱しつつ GitHub Pages 公開版を起動時に1回fetchする方式だったが、上記PR #154で置き換え済み
- **フォローアップ (未着手、記録のみ、Phase Aスコープ外として意図的見送り)**: 応答ストリーミング (SSE化)・RAG/ベクタDB・GHA WIF自動デプロイ。詳細は `docs/handoff/GOAL.md` ④ / `chatbot/README.md` 参照
