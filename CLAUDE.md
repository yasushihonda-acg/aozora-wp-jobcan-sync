...
- 採用トップのセクション順: hero → philosophy → categories → numbers → flow → faq → entry-cta (tcy.co.jp/recruit 参考)。**スタッフ紹介・育休ブログ・お知らせ/コラムの各セクションは 2026-07-14 決裁者指示で廃止**(スタッフ紹介は実写ポートレートがイラスト主体のポスタートンマナと不整合、育休ブログ/お知らせ・コラムはトップページの情報を絞り込む方針のため)
...
- **2026-07-14 追加指示 (トップページ トンマナ全面刷新)**: 決裁者指示で https://g-s.dev/ のようなポスター的トンマナへ寄せる方針を追加。①ヒーロー背景の実写 (`hero-main.jpg`) を廃止し、江口寿史風の青空+入道雲+都市スカイラインの人物なしイラストに差し替え (SCENE #13、`sky-hero.jpg` を上書き、entry-cta 背景も同一ファイル共用) ②philosophy (SCENE #11) / flow (SCENE #12) の「warm・複数人物必須」指定を撤回し、求人カード群と同一の単独キャラクター・ポスタートーンに統一。③募集職種カード (トップページ Job Categories) の実写 6 枚 (`category-*.jpg`) も求人カードイラストへ差し替え。このとき「訪問介護員 (ヘルパー)」だけ専用イラストが存在しない不整合が判明したため SCENE #14 (`illust-job-visit.png`) を新規追加、生成待ちの間は `illust-job-care-3.png` を暫定流用。詳細は `docs/specs/chatgpt-ui-prompts.md` 参照
...
- **コーポレートカラー**: ブルー `#00C4CC` (差し色) / グレー `#575656` (文字・影) / ベージュ `#f8f5ee` (背景ベース)
...
- 出力 → 本田様が UI で生成 → 私 (Claude) に送信 → 10 項目採点 → `mockup/assets/img/illust-*.png` に採用 → 全 12 枚集めて Phase 4 PR
