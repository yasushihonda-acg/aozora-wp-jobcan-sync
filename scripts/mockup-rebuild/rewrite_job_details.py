...
thumbnail img は既存 src を維持 (illust-job-{care,nurse,office,it,default}.png)。
"""
...
def rewrite_file(path: Path, job: dict) -> bool:
...
def main() -> None:
    ...
    for jid, job in jobs.items():
        ...
        if not path.exists():
            ...
            continue
        try:
            if rewrite_file(path, job):
                print(f"  OK {jid}: {job['title'][:50]}")
                ok += 1
